...
function gameFunction():void {
    inquirer
        .prompt(questionsGame)
        .then((answers) => {
            let amount = parseInt(answers.startAmount);
            const startingAmount = parseInt(answers.startAmount);
            const startingBet = parseInt(answers.startBet);
            const chosenColor = answers.chosenColor;
            let amountOfTries = 0;
            let currentBet = startingBet;
            while (amount > 0) {
                if (amount === (startingAmount + startingBet)) break;
